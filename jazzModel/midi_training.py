...
# ✅ JSONL 로드 함수 (3000개 제한)
def load_jsonl(jsonl_path, max_items=3000):
    data = []
...
# ✅ 학습 루프 (Early Stopping 추가)
def train(model, dataloader, criterion, optimizer, epochs=50, device="mps", early_stopping=5):
    model.to(device)
    model.train()
    print(f"🧠 디바이스: {device}")
    ...
    for epoch in range(epochs):
        ...
        progress_bar = tqdm(enumerate(dataloader), total=len(dataloader), desc=f"Epoch {epoch + 1}/{epochs} 진행중")
        ...
        for batch_idx, batch in progress_bar:
            ...
            progress_bar.set_postfix(loss=loss.item(), progress=f"{(batch_idx + 1) / len(dataloader) * 100:.2f}%")
        ...
        if avg_loss < best_loss:
            ...
            print(f"💾 모델 개선됨, 저장됨: {MODEL_PATH}")
        else:
            patience += 1
            print(f"⏳ Early Stopping 카운트: {patience}/{early_stopping}")
...
if __name__ == "__main__":
    print("📂 JSONL 데이터 로드 중 (3000개)")
    json_data = load_jsonl(JSONL_FILE, max_items=3000)
    ...
    criterion = nn.CrossEntropyLoss()  # ✅ 손실 함수 변경 (MIDI 이벤트 분류 개선)
